...
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600">Ajouter et gérer les patients de la clinique</p>
        </div>
    ...
    <div class="bg-white shadow rounded-xl p-6 mb-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">Ajouter un patient</h2>
        <form method="POST" action="<?php echo defined('BASE_PATH') ? constant('BASE_PATH') : ''; ?>/admin/patients.php" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="date_of_birth" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?php echo htmlspecialchars((string)($_POST['date_of_birth'] ?? '')); ?>">
            </div>
            ...
            <div>
                ...
                <input name="emergency_contact" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?php echo htmlspecialchars((string)($_POST['emergency_contact'] ?? '')); ?>">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Téléphone d'urgence</label>
                <input name="emergency_phone" type="tel" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?php echo htmlspecialchars((string)($_POST['emergency_phone'] ?? '')); ?>">
            </div>

            <div class="md:col-span-2">
                <button type="submit" class="px-6 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">Ajouter le patient</button>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white shadow rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Liste des patients</h2>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach (($patients ?? []) as $patient): ?>
                        <tr>
                            <td class="px-6 py-4 text-sm text-gray-700"><?php echo htmlspecialchars((string)($patient['id'] ?? '')); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700">
                                <?php echo htmlspecialchars((string)($patient['first_name'] ?? '')); ?> 
                                <?php echo htmlspecialchars((string)($patient['last_name'] ?? '')); ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?php echo htmlspecialchars((string)($patient['email'] ?? '')); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?php echo htmlspecialchars((string)($patient['date_of_birth'] ?? '')); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-700"><?php echo htmlspecialchars((string)($patient['phone'] ?? '')); ?></td>
                            <td class="px-6 py-4 text-sm">
                                <form method="POST" action="<?php echo defined('BASE_PATH') ? constant('BASE_PATH') : ''; ?>/admin/patients.php" class="inline">
                                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars((string)($csrf_token ?? '')); ?>">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="id" value="<?php echo htmlspecialchars((string)($patient['id'] ?? '')); ?>">
                                    <button type="submit" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce patient ?')" class="px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 text-sm">Supprimer</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
